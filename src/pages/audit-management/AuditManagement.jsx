...
const AuditManagement = () => {
    ...
    useEffect(() => {
      const fetchData = async () => {
        setIsLoading(true);
        await Promise.all([getSummary(), getAllApplications(), getAudits(), getStaffs()]);
        setIsLoading(false);
      };
                    ...
                    audits?.data?.map((audit, index) => (
                        <div key={index} className="flex items-center justify-between border rounded p-3 my-3">
                            ...
                            <div className="flex flex-col justify-between items-end">
                                <p className="text-primary-color px-[10px] py-[3px] rounded-full bg-secondary-color inline-block">in progress</p>
                                <p></p>
            ...
            <div className="mt-3 border rounded-[8px]">
                <div className="flex items-center justify-between px-7 pt-6">
                    <div className="flex items-center gap-2">
                        ...
                        <p className="text-primary-color bg-secondary-color text-[14px] px-2 py-1 rounded-full cursor-pointer">{audits?.count}</p>
                    </div>
                    ...
                    <tbody>
                        {
                            audits?.data?.map((audit, index) => (
                                <tr className="border-b" key={index}>
                                    ...
                                    <td class="px-6 py-4 text-[12px] md:text-[16px] text-[#475467]">Nigeria</td>
